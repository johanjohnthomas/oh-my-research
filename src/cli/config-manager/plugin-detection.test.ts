...
describe("addPluginToOpenCodeConfig - single package writes", () => {
  ...
  it("writes canonical plugin entry for new installs", async () => {
    ...
    expect(savedConfig.plugin).toEqual(["@johanjohnthomas/oh-my-research"])
  })
  ...
  it("updates a version-pinned legacy entry to the requested version", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("@johanjohnthomas/oh-my-research@3.16.0")
    ...
    expect(result.success).toBe(true)
    const savedConfig = JSON.parse(readFileSync(testConfigPath, "utf-8"))
    expect(savedConfig.plugin).toEqual(["@johanjohnthomas/oh-my-research@3.16.0"])
    getPluginNameWithVersionSpy.mockRestore()
  })

  it("removes stale legacy entry when canonical and legacy entries both exist", async () => {
    // given
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["@johanjohnthomas/oh-my-research", "oh-my-opencode"] }, null, 2) + "\n", "utf-8")
  ...
  it("preserves a canonical entry when the same version is re-installed", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("@johanjohnthomas/oh-my-research@3.10.0")
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["@johanjohnthomas/oh-my-research@3.10.0"] }, null, 2) + "\n", "utf-8")
    ...
    expect(result.success).toBe(true)
    const savedConfig = JSON.parse(readFileSync(testConfigPath, "utf-8"))
    expect(savedConfig.plugin).toEqual(["@johanjohnthomas/oh-my-research@3.10.0"])
    getPluginNameWithVersionSpy.mockRestore()
  })

  it("blocks a downgrade for a version-pinned canonical entry", async () => {
    // given
    const getPluginNameWithVersionSpy = spyOn(pluginNameWithVersion, "getPluginNameWithVersion").mockResolvedValue("@johanjohnthomas/oh-my-research@3.15.0")
    writeFileSync(testConfigPath, JSON.stringify({ plugin: ["@johanjohnthomas/oh-my-research@3.16.0"] }, null, 2) + "\n", "utf-8")
